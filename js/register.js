function phoneRegister(){
	...
	getcode.onclick = e=>{
		...
		ajax({
			...
			if(data.ok==1){
				inputred(phone);
				targetspan(phone).innerHTML = "该号码已存在";
	...
	function regist(){
		...
		if(phone.value == "")notnull(phone);
			...
			var phoneReg = /^(\+86|0086)?\s*1[3578]\d{9}$/;
			if(!phoneReg.test(phone.value)) notright(phone,"手机");
			else pass(phone);
		}
		//PHP验证码
		...
		if(canSubmit){
			console.log(`uname=${uname.value}&upwd=${upwd.value}&sex=${sex.value}&phone=${phone.value}`);
			ajax({
				type:"post",
